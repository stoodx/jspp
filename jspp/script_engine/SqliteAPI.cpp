...
{
	m_pSqliteAPI = this;
	PUSH_C_FUNCTION(open_database, 1);
	PUSH_C_FUNCTION(close_database, 1);
	PUSH_C_FUNCTION(exec_database, 3);
	PUSH_C_FUNCTION( read_database_result, 0);
}
...
bool SqliteAPI::execDatabase(const std::string&  strStatemenet,  bool bUseCallback)
{
	if (m_status != Status::DATABASE_OPEN)
	{
	...
	getTableName(strStatemenet, m_strTableName); //for json

	int nRes = 0;
	char* strErr = NULL;
	if (bUseCallback)
		nRes = sqlite3_exec(m_pDb, strStatemenet.c_str(), readSqlResponse_callback,  0, &strErr);
	else
		nRes = sqlite3_exec(m_pDb, strStatemenet.c_str(), NULL,  0, &strErr);
...
duk_ret_t SqliteAPI::open_database(duk_context *ctx)
{
...
duk_ret_t SqliteAPI::close_database(duk_context *ctx)
{
	LOG_MSG(ILogger::default(), LOG_LEVEL_DEBUG, "SqliteAPI::close_database()");
...
duk_ret_t SqliteAPI::exec_database(duk_context *ctx)
{
	LOG_MSG(ILogger::default(), LOG_LEVEL_DEBUG, "SqliteAPI::exec_database()");
	SqliteAPI* pSqliteAPI = (SqliteAPI*)duk_require_pointer(ctx, 0);
...
duk_ret_t SqliteAPI::read_database_result(duk_context *ctx)
{
	LOG_MSG(ILogger::default(), LOG_LEVEL_DEBUG, "SqliteAPI::read_database_resul()");
	if (!m_pSqliteAPI)
	{
		LOG_MSG(ILogger::default(), LOG_LEVEL_WARN, "SqliteAPI - error by init");
		duk_push_null(ctx);;
...
int SqliteAPI::readSqlResponse_callback(void *notUsed, int argc, char **strRow, char **strColName)
{
	if (!m_pSqliteAPI)
		return 1;
